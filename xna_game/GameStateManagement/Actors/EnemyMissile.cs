...
{
    public class EnemyMissile : Actor
    {
        ...
        public EnemyMissile(Game game, Actor Firee, Ship Target, int i)
            : base(game)
        {
            sMeshName = "Missile";
            ...
            //Rotation *= Quaternion.CreateFromAxisAngle(Vector3.UnitY, float(MathHelper.PiOver2);
            Direction = Target.Position - Firee.Position;
            if (index == 1)
                Direction.Z -= 450;
            Direction.Normalize();

            //Rotation *= Quaternion.CreateFromAxisAngle(Vector3.UnitY, (float)MathHelper.PiOver2);
            //Rotation *= Quaternion.CreateFromAxisAngle(Target.Position, 0.0f);
            ...
            if (index == 1)
                ...
                Force = Vector3.Multiply(Direction, 0.5f);
            Velocity = Force;
        ...
        {
            if (WorldBounds.Intersects(ship.WorldBounds))
            {
                ship.setInvincible();
                SelfDestroy();
                return;
