...
{
    public class PlayerMissile : Actor
    {
        Random rand = new Random();
        Utils.Timer selfDestroy = new Utils.Timer();
        private Vector3 fireDirection;

        public PlayerMissile(Game game)
            : base(game)
        {
            sMeshName = "Missile";
        ...
        public PlayerMissile(Game game, Ship Firee)
            : base(game)
        {
        ...
        public override void Initialize()
        {
            ...
            Mass = 1.0f;
            selfDestroy.AddTimer("Self-Destroy", 1.0f, new Utils.TimerDelegate(SelfDestroy), false);
        }
        ...
        protected override void CheckCollision()
        {
            foreach (Asteroids A in SpawnManager.asteroidList)
            {
                if (WorldBounds.Intersects(A.WorldBounds))
                {
                    ...
                    SpawnManager.asteroidList.Remove(A);
                    A.stopFiring();
                    SelfDestroy();
                    return;
                }
